 ...
 Start
  = gs:GlobalSection _? ds1:DataSection? _? ts:TextSection _? ds2:DataSection? {
    ...
    newPath(idRoot, 'Start', [gs, ds1, ts, ds2]);
     new Root(gs, dataSectionConcat, ts, cst);
  }
...
TextSection
  = ".section"? ".text"? _? ident:label ":" _* ins:linea*
  {
    ...
    newPath(idRoot, 'TextSection', [{ id:idInst }]);
     new TextSection(idRoot, 'TextSection', ident, ins);
  }
  

linea =  ins:instruccion _*  {  ins }
	  / sss:comentario _*  {  }
      / etiq:etiq      _*  {  }
...
mov_inst
    = mov:"mov" _* r1:reg64 "," _* r2:(reg64 / immediate)*  
    {
    const loc = location()?.start;
    const idRoot = cst.newNode();
    newPath(idRoot, 'Move', ['mov', r1, 'COMA', r2]);
     new Move(loc?.line, loc?.column, idRoot, r1.name, r2.name);
    }
...
string "string"
  = "\"" chars:[^\"]* "\"" _ 
  {
    ...
    newPath(idRoot, 'string', [chars.join('')]);
     new Primitive(loc?.line, loc?.column, idRoot, Type.ASCIZ, chars.join(''));
  }
...
entero = arg: [0-9]+    
{
    ...
    newPath(idRoot, 'integer', [text()]);
     new Primitive(loc?.line, loc?.column, idRoot, Type.WORD, parseInt(text(), 10));
  }
